...
The package includes a sample data set, named 'Microbiome_dat', which can be loaded directly. `Microbiome_dat` is a list containing four arguments, namely `pheno`: a `data.frame` of covariates containing a main effect variable in the 1st column and a confounding variable in the 2nd column; `counts`: a `matrix` of the taxonomic read counts containing 80 samples and 40 taxa (samples in columns and taxa in rows); `batch`: a numeric vector labeling the batch of each sample; `continuous`: a numeric vector indicating whether the variables are continuous($=0$) or catagorical($\neq 0$).  
```{r results='hide', message=FALSE, warning=FALSE}
library(BDMMAcorrect)
```
```{r, echo=T}
data(Microbiome_dat)
names(Microbiome_dat)
attach(Microbiome_dat)
```
...
figure = VBatch(counts, batch = batch, method = "bray")
print(figure)
```
...
figure = VBatch(counts, batch = batch, main_variable = main_variable, method = "bray")
print(figure[[1]])
```
...
```{r, echo=TRUE}
output <- BDMMA(pheno, counts, batch, continuous, burn_in = 4000, sample_period = 4000)
